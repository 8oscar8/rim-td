...
class App {
  ...
  gamble(type) {
    ...
    if (type === 'wood') {
        // 목재 도박 (10:1 교환비 조정)
        if (rand < 0.02) { s.component += 1; result = { msg: "목재 정제 중 부품 발견! (+1 부품)", grade: "Legendary" }; }
        else if (rand < 0.08) { s.uranium += 1; result = { msg: "미량의 우라늄 채취 (+1 우라늄)", grade: "Rare" }; }
        else if (rand < 0.25) { s.plasteel += 3; result = { msg: "목재에서 플라스틸 정제 (+3 플라스틸)", grade: "Uncommon" }; }
    } else if (type === 'steel') {
        // 강철 도박 (10:1 교환비 조정)
        if (rand < 0.02) { s.jade += 1; result = { msg: "정제된 비취 결정 발견! (+1 비취)", grade: "Legendary" }; }
        else if (rand < 0.08) { s.component += 1; result = { msg: "강철 속 부품 회수 (+1 부품)", grade: "Epic" }; }
        else if (rand < 0.25) { s.plasteel += 6; result = { msg: "강철 분해로 플라스틸 확보 (+6 플라스틸)", grade: "Uncommon" }; }
    } else if (type === 'silver') {
        // 은화 도박 (10:1 교환비 조정)
        if (rand < 0.01) { s.silver += 1000; result = { msg: "[암시장 잭팟] 대형 사고! (+1000 은)", grade: "Mythic" }; }
        else if (rand < 0.04) { s.component += 1; result = { msg: "암시장에서 부품 획득 (+1 부품)", grade: "Legendary" }; }
        else if (rand < 0.15) { s.plasteel += 4; result = { msg: "암시장 플라스틸 거래 (+4 플라스틸)", grade: "Uncommon" }; }
    }
